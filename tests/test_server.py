...
async def test_get_cards_by_due_and_deck(mock_anki):
    ...
    mock_anki.find_cards.assert_called_with(query="is:due -is:suspended prop:due<=0")
    assert cards == [1, 2, 3]

    # Test with deck filter
    cards = await get_cards_by_due_and_deck(deck="Default")
    mock_anki.find_cards.assert_called_with(query="is:due -is:suspended prop:due<=0 deck:Default")

    # Test with day parameter
    cards = await get_cards_by_due_and_deck(day=1)
    mock_anki.find_cards.assert_called_with(query="is:due -is:suspended prop:due<2")
